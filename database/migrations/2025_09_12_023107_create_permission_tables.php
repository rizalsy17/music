...
return new class extends Migration
{
    public function up(): void
    {
        ...
        throw_if($teams && empty($columnNames['team_foreign_key'] ?? null), new Exception('Error: team_foreign_key not set.'));

        // permissions
        Schema::create($tableNames['permissions'], static function (Blueprint $table) {
            $table->bigIncrements('id'); // INTEGER
            $table->string('name');
        ...
        Schema::create($tableNames['model_has_permissions'], static function (Blueprint $table) use ($tableNames, $columnNames, $pivotPermission, $teams) {
            $table->unsignedBigInteger($pivotPermission); // FK ke permissions (integer)
            $table->string('model_type');
            $table->uuid($columnNames['model_morph_key']); // UUID untuk users
        ...
        Schema::create($tableNames['model_has_roles'], static function (Blueprint $table) use ($tableNames, $columnNames, $pivotRole, $teams) {
            $table->unsignedBigInteger($pivotRole); // FK ke roles (integer)
            $table->string('model_type');
            $table->uuid($columnNames['model_morph_key']); // UUID untuk users
        ...
        Schema::create($tableNames['role_has_permissions'], static function (Blueprint $table) use ($tableNames, $pivotRole, $pivotPermission) {
            $table->unsignedBigInteger($pivotPermission); // INTEGER
            $table->unsignedBigInteger($pivotRole);       // INTEGER
